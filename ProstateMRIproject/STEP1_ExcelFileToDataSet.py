...
def makeCSVofCategoryDefinitionsForColumn(catDict,columnCode):
    fileName = 'Column_'+columnCode+'_Dictionary.csv'
    fullPath = os.path.join(outputPath,fileName)
    csvOpen = open(fullPath,'w')
    csvOpen.write('ColumnNumber/CategoryNumber,CorrespondingStringInSpreadsheet\n')
    for spreadsheetStr in catDict:
        catValue = catDict[spreadsheetStr]
        csvOpen.write(str(catValue) + "," + str(spreadsheetStr) + "\n")
    csvOpen.close()
...
makeCSVofCategoryDefinitionsForColumn(colERdictionary,'ER')
...
makeCSVofCategoryDefinitionsForColumn(colEVdictionary,'EV')
...
colFIfeatureMatrix,colFIfeatureVector,colFIdictionary = \
    CategoryFeatureLogic.autoGeneratedCategoryToOneHot(
    colEVstrings,missingDataStrings_colER_EV_FI)
makeCSVofCategoryDefinitionsForColumn(colFIdictionary,'FI')
